...
class CLIHandler:
    ...
    def parse_arguments(self):
        ...
        # For toggle-bar command
            ...
            arch = update_service.architecture_display
            version_message = (
                f"YASB Reborn v{YASB_VERSION} {arch} ({YASB_RELEASE_CHANNEL})\nYASB-CLI v{YASB_CLI_VERSION}"
            )
...
class CLIUpdateHandler:
    ...
    def update_yasb(self, yasb_version: str):
        ...
        if not update_service.is_update_supported():
            ...
            print(f"Architecture: {update_service.architecture_display}")
            if update_service.architecture.name == "UNKNOWN":
                print("Reason: Unsupported architecture")
            sys.exit(1)

        print("\nChecking for updates...")
        print(f"Current version: {yasb_version} ({update_service.architecture_display})")
