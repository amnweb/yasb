...
from core.validation.widgets.yasb.weather_api import VALIDATION_SCHEMA
...
class WeatherApiWidget(BaseWidget):
    ...
    def __init__(
        ...
        super().__init__((update_interval * 1000), class_name="weatherapi-widget")
                ...
                if isinstance(active_widgets[widget_index], QLabel):
                    if '<span' in part and '</span>' in part:
                        ...
                        # Retrieve current class and append new class based on weather conditions
                            ...
                            append_class_icon = self.weather_data.get('{conditions}', "").lower().replace(" ", "-")
