...
class WorkspaceButtonWithIcons(QFrame):
    ...
    def update_icons(self):
        if not self.parent_widget._app_icons_enabled_active and self.workspace_index == self.parent_widget._curr_workspace_index:
            self.icons = []
        elif self.parent_widget._app_icons_enabled_active and not self.parent_widget._app_icons_enabled and self.workspace_index != self.parent_widget._curr_workspace_index:
            self.icons = []
        else:
            ...
            if self.parent_widget._app_icons_max > 0:
                self.icons = self.icons[:self.parent_widget._app_icons_max]

        existing_icon_labels = [self.button_layout.itemAt(i).widget() for i in range(self.button_layout.count()) if self.button_layout.itemAt(i).widget().property("class").startswith("icon")]

        # Remove extra QLabel widgets if there are more than needed
        for i in range(len(self.icons), len(existing_icon_labels)):
            extra_label = existing_icon_labels[i]
            self.button_layout.removeWidget(extra_label)
            extra_label.setParent(None)
            
        # Add or update icons
        for index, icon in enumerate(self.icons):
            if index < len(existing_icon_labels):
                existing_icon_labels[index].setPixmap(icon)
            else:
                icon_label = QLabel()
...
class WorkspaceWidget(BaseWidget):
    ...
    def __init__(
        ...
        self._app_icons_enabled = app_icons['enabled']
        self._app_icons_enabled_active = app_icons['enabled_active']
        self._app_icons_size = app_icons['size']
        self._app_icons_max = app_icons['max_icons']
        self._app_icons_hide_duplicates = app_icons['hide_duplicates']
        self._app_icons_hide_floating = app_icons['hide_floating']
    ...
    def _on_komorebi_update_event(self, event: dict, state: dict) -> None:
        if self._update_komorebi_state(state):     
            ...
            if self._app_icons_enabled or self._app_icons_enabled_active:
                try:
    ...
    def _update_komorebi_state(self, komorebi_state: dict) -> bool:
        try:
            ...
            if self._komorebi_state:
                ...
                if self._app_icons_enabled or self._app_icons_enabled_active:
                    self._curr_num_windows_in_workspaces = self._curr_num_windows_in_workspaces[:len(self._komorebi_workspaces)] + [0] * (len(self._komorebi_workspaces) - len(self._curr_num_windows_in_workspaces))
    ...
    def _try_add_workspace_button(self, workspace_index: int) -> WorkspaceButton:
        ...
        if workspace_index not in workspace_button_indexes:
            ...
            if self._app_icons_enabled or self._app_icons_enabled_active:
                workspace_btn = WorkspaceButtonWithIcons(workspace_index, self, default_label, active_label, populated_label, self._animation)
            else:
    ...
    def _get_all_windows_in_workspace(self, workspace_index: int) -> List[dict] | None:
        ...
        if not self._app_icons_hide_floating:
    ...
    def _get_app_icon(self, hwnd: int) -> QPixmap | None:
        try:
            ...
            if self._app_icons_hide_duplicates:
            ...
            if cache_key in self._icon_cache:
                ...
                if icon_img:
                    icon_img = icon_img.resize(
                        (int(self._app_icons_size  * self.dpi), int(self._app_icons_size * self.dpi)),
                        Image.LANCZOS
                    ).convert("RGBA")
                else:
