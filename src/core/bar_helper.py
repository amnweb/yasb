...
from PyQt6.QtWidgets import (QCheckBox, QFrame, QHBoxLayout, QMenu,
                            QSizePolicy, QWidget, QWidgetAction)
...
class AutoHideManager(QObject):
    ...
    def _is_mouse_in_safe_zone(self, cursor_pos, bar_geometry):
        ...
        # Convert global cursor position to screen-relative position
        ...
        if alignment['position'] == "top":
            # For top bar, safe zone is between detection zone (y=0) and bar top
            bar_top = bar_geometry.y() - screen_geometry.y()
            return 0 <= screen_y <= bar_top
        else:
            # For bottom bar, safe zone is between bar bottom and detection zone
...
class FullscreenManager(QObject):
    ...
    def _check_fullscreen(self):
        ...
        dpi = self.bar_widget.screen().devicePixelRatio()
        ...
        if self._prev_fullscreen_state == is_fullscreen:
            return

        self._prev_fullscreen_state = is_fullscreen
        if is_fullscreen and self.bar_widget.isVisible():
            self.bar_widget.hide()
        elif not is_fullscreen and not self.bar_widget.isVisible():
            self.bar_widget.show()
...
class BarContextMenu:
    ...
    MENU_STYLE = """
        ...
        QMenu::item {
            ...
            font-size: 11px; font-family: 'Segoe UI'; font-weight: 600;
        }
    ...
    CHECKBOX_STYLE = """
        QCheckBox {
            ...
            font-size: 11px; margin: 0; color: #fff;font-family: 'Segoe UI';font-weight: 600;
        }
    ...
    def show(self, position):
        ...
        current_autohide_enabled = (hasattr(self.parent, '_autohide_manager') and 
                                   self.parent._autohide_manager and 
                                   self.parent._autohide_manager.is_enabled())
    ...
    def _on_menu_about_to_hide(self):
        ...
        try:
            ...
            if (hasattr(self.parent, '_autohide_manager') and 
                self.parent._autohide_manager and 
                self.parent._autohide_manager.is_enabled()):
        ...
        for i, layout_type in enumerate(['left', 'center', 'right']):
            ...
            if self._widgets.get(layout_type):
                for widget in self._widgets[layout_type]:
                    self._add_widget_checkbox(widgets_menu, widget, layout_type)
            else:
                no_widgets = widgets_menu.addAction("  No active widgets")
                no_widgets.setEnabled(False)
    ...
    def _add_widget_checkbox(self, menu, widget, layout_type):
    ...
    def _get_widget_display_name(self, widget):
        for layout_type, widget_list in self._widgets.items():
            try:
                ...
                if (self._widget_config_map and layout_type in self._widget_config_map 
    ...
    def _take_screenshot(self):
        ...
        try:
            ...
            padding_bottom = 10  # Fixed 10px bottom padding as requested
            ...
            if bar_alignment['position'] == 'top':
                ...
                screenshot_height = (bar_geometry.y() - screen_geometry.y()) + bar_geometry.height() + padding_bottom
            else:
            ...
            screenshots_dir = os.path.join(os.path.expanduser("~"), "Pictures", "YASB Screenshots")
        ...
        try:
            ...
            if not self.parent._autohide_manager.is_enabled():
                self.parent._autohide_manager.setup_autohide(
                    self.parent._alignment, 
                    self.parent._padding
                )
