                    ...
                    while self._active:
                        ...
                        if result == WAIT_OBJECT_0:  # Change notification was signaled
                            ...
                            if bin_info:
                                ...
                                if (bin_info["size_bytes"] != self._last_info["size_bytes"] or 
                                    bin_info["num_items"] != self._last_info["num_items"]):
                            ...
                            if not kernel32.FindNextChangeNotification(change_handle):
                                logging.error(f"Failed to reset change notification for {recycle_bin_path}: {ctypes.WinError()}")
                                break
                        else:
                            # Error occurred
